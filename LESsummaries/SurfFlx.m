function vals = SurfFlx(dagnc,trange)
 ...
 vals.u_star   = sqrt(vals.stress/1000);
 ...
 vals.lam      = flxvars.wave_l;
 vals.e        = -flxvars.lhf_top./(rho.*LH);
 vals.netp     = flxvars.rain-vals.e;
 vals.Salflx   = SalTSG.*(vals.netp*(1e-3)/3600);
 vals.LaBflx   = vals.S_0.*(4*pi./flxvars.wave_l).*vals.stress./rho;
 vals.JhBflx   = alpha.*g.*vals.Jh./(Cp.*rho);
 vals.SaBflx   = -beta.*g.*vals.Salflx;
