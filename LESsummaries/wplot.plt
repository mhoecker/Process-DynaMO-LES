...
set parametric
set xrange [0:2*xytic+1]
set yrange [0:2*xytic+1]
set zrange [-128:0]
set ztics out 64 offset 2,0
...
set bmargin at screen bloc(row)
splot \
datdir.name.'yz010'.ti.'.dat' binary matrix u (000):1:2:3 w image not,\
datdir.name.'xz030'.ti.'.dat' binary matrix u 1:(2*xytic):2:3 w image not,\
datdir.name.'xy010'.ti.'.dat' binary matrix u 1:2:(-5):3 every :::(ijtic-1) w image not,\
datdir.name.'xz020'.ti.'.dat' binary matrix u 1:(1*xytic):2:3 w image not,\
datdir.name.'xz010'.ti.'.dat' binary matrix u 1:(0*xytic):2:3 w image not,\
datdir.name.'yz030'.ti.'.dat' binary matrix u (2*xytic):1:2:3 every ::(ijtic-1) w image not
unset multiplot
#
#name = "tke"
