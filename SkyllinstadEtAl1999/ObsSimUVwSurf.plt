...
datdir.abrev."JPtau.dat" binary format="%f%f%f%f%f"u 1:5 axes x1y1 title "{/Symbol t}_y" lc rgbcolor "grey"
#
...
set cblabel "u,v (m/s)" offset -yloff/2
cbmin = UVmin
cbmax = UVmax
...
set tmargin at screen tloc(row)
set bmargin at screen bloc(row)
set colorbox user origin rloc(col),bloc(row)+0.075*vskip size .1*(1-rloc(col)),3.85*vskip
